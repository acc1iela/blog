...
本記事では、v18 との差分を中心に何が変わり、何に注意すべきかを整理する。
...
React19では**React Compiler**（旧称: React Forget）が登場した。コンパイル時に自動でメモ化を行うため、以下を手書きするケースが激減する見込み。
...
破壊的変更は主に「v18で既に非推奨だったもの」なので、v19で警告を潰してきたプロジェクトなら移行コストは低い。一方、古いコードベースでは `ReactDOM.render`やString refsがまだ残っているケースもあるので、まずcodemodを当てて確認するのが堅実な進め方。
